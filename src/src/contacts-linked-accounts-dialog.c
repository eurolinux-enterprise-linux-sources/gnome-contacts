/* contacts-linked-accounts-dialog.c generated by valac 0.26.0.27-f1f8, the Vala compiler
...
static void ____lambda75_ (Block30Data* _data30_, GtkWidget* w);
...
static void ____lambda76_ (Block30Data* _data30_);
...
static void __lambda77_ (Block30Data* _data30_, GObject* obj, GAsyncResult* _result_);
...
static void block29_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data29_->_ref_count_)) {
		ContactsLinkedAccountsDialog* self;
		self = _data29_->self;
		_g_object_unref0 (_data29_->contact);
...
static void block30_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data30_->_ref_count_)) {
		...
		_g_object_unref0 (_data30_->row_grid);
		_g_object_unref0 (_data30_->p);
...
static void ____lambda75_ (Block30Data* _data30_, GtkWidget* w) {
	...
	_tmp0_ = w;
	_tmp1_ = _data29_->contact;
	_tmp2_ = _tmp1_->individual;
	_tmp3_ = _data29_->contact;
...
static void ____lambda76_ (Block30Data* _data30_) {
	...
	_data29_ = _data30_->_data29_;
	self = _data29_->self;
	_tmp0_ = _data29_->contact;
	_tmp1_ = _data30_->p;
	contacts_unlink_persona (_tmp0_, _tmp1_, ___lambda77__gasync_ready_callback, block30_data_ref (_data30_));
}
...
ContactsLinkedAccountsDialog* contacts_linked_accounts_dialog_construct (GType object_type, ContactsContact* contact) {
	...
	is_first = TRUE;
	{
		...
		while (TRUE) {
			...
			_data30_->p = (FolksPersona*) _tmp53_;
			_tmp54_ = is_first;
			if (_tmp54_) {
				is_first = FALSE;
				block30_data_unref (_data30_);
			...
			_tmp60_ = _data29_->contact;
			_tmp61_ = image_frame;
			contacts_contact_keep_widget_uptodate (_tmp60_, (GtkWidget*) _tmp61_, _____lambda75__gtk_callback, block30_data_ref (_data30_), block30_data_unref);
			_tmp62_ = _data30_->row_grid;
			_tmp63_ = image_frame;
			...
			_tmp94_ = _data30_->row_grid;
			_tmp95_ = button;
			gtk_grid_attach (_tmp94_, (GtkWidget*) _tmp95_, 2, 0, 1, 2);
			_tmp96_ = button;
			g_signal_connect_data (_tmp96_, "clicked", (GCallback) _____lambda76__gtk_button_clicked, block30_data_ref (_data30_), (GClosureNotify) block30_data_unref, 0);
			_tmp97_ = _data30_->row_grid;
			gtk_widget_show_all ((GtkWidget*) _tmp97_);
			_tmp98_ = self->priv->linked_accounts_view;
			_tmp99_ = _data30_->row_grid;
