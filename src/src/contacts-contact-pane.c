/* contacts-contact-pane.c generated by valac 0.26.0.27-f1f8, the Vala compiler
...
static void __lambda71_ (Block24Data* _data24_, GtkWidget* w);
...
static void __lambda72_ (Block24Data* _data24_);
...
static void __lambda73_ (Block25Data* _data25_, GObject* obj, GAsyncResult* _result_);
...
static void __lambda74_ (Block24Data* _data24_);
...
static void ___lambda81_ (Block26Data* _data26_);
...
static void ____lambda82_ (Block27Data* _data27_);
...
static void ___lambda84_ (Block28Data* _data28_);
...
static void block25_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data25_->_ref_count_)) {
		...
		_g_free0 (_data25_->linked_contact);
...
static void __lambda73_ (Block25Data* _data25_, GObject* obj, GAsyncResult* _result_) {
	...
	operation = _tmp1_;
	g_signal_emit_by_name (self, "contacts-linked", NULL, _data25_->linked_contact, operation);
...
static void __lambda72_ (Block24Data* _data24_) {
	...
	_data25_->linked_contact = _tmp3_;
	_tmp4_ = self->contact;
	_tmp5_ = _data24_->c;
	contacts_link_contacts (_tmp4_, _tmp5_, ___lambda73__gasync_ready_callback, block25_data_ref (_data25_));
...
void contacts_contact_pane_add_suggestion (ContactsContactPane* self, ContactsContact* c) {
	...
	_tmp18_ = _data24_->c;
	_tmp19_ = image_frame;
	contacts_contact_keep_widget_uptodate (_tmp18_, (GtkWidget*) _tmp19_, ___lambda71__gtk_callback, block24_data_ref (_data24_), block24_data_unref);
	...
	no = _tmp56_;
	g_signal_connect_data (yes, "clicked", (GCallback) ___lambda72__gtk_button_clicked, block24_data_ref (_data24_), (GClosureNotify) block24_data_unref, 0);
	g_signal_connect_data (no, "clicked", (GCallback) ___lambda74__gtk_button_clicked, block24_data_ref (_data24_), (GClosureNotify) block24_data_unref, 0);
...
static void block26_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data26_->_ref_count_)) {
		ContactsContactPane* self;
		self = _data26_->self;
		_g_object_unref0 (_data26_->dialog);
		_g_object_unref0 (self);
...
static void ____lambda83_ (ContactsContactPane* self, GObject* obj, GAsyncResult* res) {
	...
	__catch9_g_error:
	{
		...
		_g_object_unref0 (_data28_->dialog);
		_data28_->dialog = (GtkDialog*) _tmp8_;
		_g_error_free0 (e);
	}
	__finally9:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	if (_tmp13_ == NULL) {
		...
		_g_object_unref0 (_data28_->dialog);
		_data28_->dialog = (GtkDialog*) _tmp16_;
	}
	contacts_contact_pane_set_edit_mode (self, FALSE, TRUE);
	_tmp17_ = _data28_->dialog;
	if (_tmp17_ != NULL) {
		GtkDialog* _tmp18_ = NULL;
		GtkDialog* _tmp19_ = NULL;
		_tmp18_ = _data28_->dialog;
		g_signal_connect_data (_tmp18_, "response", (GCallback) ____lambda84__gtk_dialog_response, block28_data_ref (_data28_), (GClosureNotify) block28_data_unref, 0);
		_tmp19_ = _data28_->dialog;
...
void contacts_contact_pane_create_contact (ContactsContactPane* self) {
	...
	if (_tmp45_ == ((guint) 0)) {
		...
		_data26_->dialog = (GtkDialog*) _tmp48_;
		_tmp49_ = _data26_->dialog;
		gtk_widget_show ((GtkWidget*) _tmp49_);
		_tmp50_ = _data26_->dialog;
		g_signal_connect_data (_tmp50_, "response", (GCallback) ____lambda81__gtk_dialog_response, block26_data_ref (_data26_), (GClosureNotify) block26_data_unref, 0);
		...
		if (_tmp56_ == NULL) {
			...
			_data27_->dialog = (GtkDialog*) _tmp59_;
			_tmp60_ = _data27_->dialog;
			gtk_widget_show ((GtkWidget*) _tmp60_);
			_tmp61_ = _data27_->dialog;
			g_signal_connect_data (_tmp61_, "response", (GCallback) _____lambda82__gtk_dialog_response, block27_data_ref (_data27_), (GClosureNotify) block27_data_unref, 0);
