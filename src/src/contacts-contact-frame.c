/* contacts-contact-frame.c generated by valac 0.26.0.27-f1f8, the Vala compiler
...
static void ___lambda27_ (ContactsContactFrame* self);
...
static void ____lambda27__gtk_button_clicked (GtkButton* _sender, gpointer self) {
	___lambda27_ ((ContactsContactFrame*) self);
}


static gboolean _contacts_contact_frame_draw_image_gtk_widget_draw (GtkWidget* _sender, cairo_t* cr, gpointer self) {
	gboolean result;
	result = contacts_contact_frame_draw_image ((ContactsContactFrame*) self, cr);
	return result;
}
...
ContactsContactFrame* contacts_contact_frame_construct (GType object_type, gint size, gboolean with_button) {
	...
	if (_tmp5_) {
		...
		button = _tmp6_;
		_tmp7_ = button;
		_tmp8_ = gtk_widget_get_accessible ((GtkWidget*) _tmp7_);
		_tmp9_ = _ ("Change avatar");
		atk_object_set_name (_tmp8_, _tmp9_);
		_tmp10_ = button;
		_tmp11_ = gtk_widget_get_style_context ((GtkWidget*) _tmp10_);
		gtk_style_context_add_class (_tmp11_, "contacts-square");
		_tmp12_ = button;
		gtk_button_set_relief (_tmp12_, GTK_RELIEF_NONE);
		_tmp13_ = button;
		gtk_button_set_focus_on_click (_tmp13_, FALSE);
		_tmp14_ = button;
		_tmp15_ = image;
		gtk_container_add ((GtkContainer*) _tmp14_, (GtkWidget*) _tmp15_);
		_tmp16_ = button;
		g_signal_connect_object (_tmp16_, "clicked", (GCallback) ____lambda27__gtk_button_clicked, self, 0);
		_tmp17_ = button;
	...
	_tmp20_ = image;
	g_signal_connect_object ((GtkWidget*) _tmp20_, "draw", (GCallback) _contacts_contact_frame_draw_image_gtk_widget_draw, self, 0);
...
void contacts_contact_frame_set_image (ContactsContactFrame* self, FolksAvatarDetails* details, ContactsContact* contact) {
	...
	_tmp1_ = details;
	if (_tmp1_ != NULL) {
		FolksAvatarDetails* _tmp2_ = NULL;
	...
	if (_tmp0_) {
		{
			...
			FolksAvatarDetails* _tmp5_ = NULL;
			GLoadableIcon* _tmp6_ = NULL;
			GLoadableIcon* _tmp7_ = NULL;
			gint _tmp8_ = 0;
			GInputStream* _tmp9_ = NULL;
			...
			_tmp5_ = details;
			_tmp6_ = folks_avatar_details_get_avatar (_tmp5_);
			_tmp7_ = _tmp6_;
			_tmp8_ = self->priv->size;
			_tmp9_ = g_loadable_icon_load (_tmp7_, _tmp8_, NULL, NULL, &_inner_error_);
			stream = _tmp9_;
		...
		font_size = (_tmp7_ - 4) + 1;
		{
			...
			while (TRUE) {
				...
				if (!_tmp8_) {
					...
					_tmp11_ = self->priv->size;
					if (!(_tmp10_ > (_tmp11_ * PANGO_SCALE))) {
						break;
					}
				}
				_tmp8_ = FALSE;
				...
				_tmp14_ = fd;
				_tmp15_ = font_size;
				pango_font_description_set_absolute_size (_tmp14_, (gdouble) (_tmp15_ * PANGO_SCALE));
				_tmp16_ = self->priv->layout;
				_tmp17_ = fd;
				pango_layout_set_font_description (_tmp16_, _tmp17_);
...
GType contacts_contact_frame_get_type (void) {
	...
	if (g_once_init_enter (&contacts_contact_frame_type_id__volatile)) {
		...
		contacts_contact_frame_type_id = g_type_register_static (gtk_frame_get_type (), "ContactsContactFrame", &g_define_type_info, 0);
